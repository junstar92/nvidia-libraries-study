 ...
 *              
 * Compile:     nvcc -O3 -o vector_add_unified_memory vector_add_unified_memory.cu
 * Run:         ./vector_add <n>
...
int main(int argc, char** argv)
{
    ...
    /*************** Case 2: using unified memory ***************/
    total = 0.f;
    // allocate unified memory
    float *um_a, *um_b, *um_c;
    CUDA_ERROR_CHECK(cudaMallocManaged(&um_a, bytes));
    CUDA_ERROR_CHECK(cudaMallocManaged(&um_b, bytes));
    CUDA_ERROR_CHECK(cudaMallocManaged(&um_c, bytes));
    ...
    // free memory
    CUDA_ERROR_CHECK(cudaFree(um_a));
    CUDA_ERROR_CHECK(cudaFree(um_b));
    CUDA_ERROR_CHECK(cudaFree(um_c));
    free(host_ref);

    CUDA_ERROR_CHECK(cudaEventElapsedTime(&msec, start, stop));
    printf("> vectorAddKernel(unified memory) Elapsed Time: %f ms\n", total / 100);

 ...
 *              in a constant memory array with read-only cache
 *              
 * Compile:     nvcc -O3 -o cmem_readonly cmem_readonly.cu
 * Run:         ./cmem_readonly
 *****************************************************************************/
...
int main(int argc, char** argv)
{
    ...
    CUDA_ERROR_CHECK(cudaMalloc(&d_coef, (RADIUS + 1) * sizeof(float)));

    // set up coefficient to global memory
    float const h_coef[] = {a0, a1, a2, a3, a4};
    CUDA_ERROR_CHECK(cudaMemcpy(d_coef, h_coef, (RADIUS + 1) * sizeof(float), cudaMemcpyHostToDevice));
    ...
    // launch stencil1DReadOnly() kernel
    CUDA_ERROR_CHECK(cudaEventRecord(start));
    stencil1DReadOnly<<<grid, block>>>(d_in + RADIUS, d_out + RADIUS, d_coef, num_elements);
    CUDA_ERROR_CHECK(cudaEventRecord(end));
    CUDA_ERROR_CHECK(cudaEventSynchronize(end));
    CUDA_ERROR_CHECK(cudaEventElapsedTime(&msec, start, end));

    printf("stencil1DReadOnly <<< %4d, %4d >>> elapsed time: %f ms\n", grid.x, block.x, msec);

/*****************************************************************************
 * File:        write_segment.cu
 * Description: This code is for demonstrates the impact of misaligned writes 
 *              on performance by forcing misaligned writes to occur on a float*.
 *              writeOffset kernel function is launched with <<<1, 32>>> and 
 *              the number of elements is 128 (512 bytes)
 * 
 *              To check the effect of misaligned write, use 'nsight compute'
 *              with 'l1tex__t_sectors_pipe_lsu_mem_global_op_st.sum' and
 *              'smsp__sass_average_data_bytes_per_sector_mem_global_op_st.pct' metrics.
 *              The first metric shows the number of global memory store transaction and
 *              the second metric shows the efficiency of global memory store.
 * 
 * Compile:     nvcc -O3 -o write_segment write_segment.cu
 * Run:         ./write_segment [offset]
 *****************************************************************************/
#include <stdio.h>
